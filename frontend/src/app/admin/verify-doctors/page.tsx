 ...
 * Interface pour approuver les comptes docteurs
 */
...
import {
  Heart, LogOut, Bell, Shield, CheckCircle, XCircle,
...
export default function AdminVerifyDoctors() {
  ...
  const handleVerifyDoctor = async (doctorId: number) => {
    ...
    try {
      ...
      setDoctors(doctors.map(doc =>
        doc.id === doctorId ? { ...doc, is_verified: true } : doc
      ));
      ...
      logger.error('Failed to verify doctor', {
        adminId: user?.id,
        doctorId,
        errorMessage: error?.message,
      }, error);
  ...
  const handleRejectDoctor = async (doctorId: number) => {
    ...
    try {
      ...
      logger.info('Doctor rejected and deleted', {
        adminId: user?.id,
        doctorId,
      });
    } catch (error: any) {
